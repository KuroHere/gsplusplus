...
public enum ClientEventManager implements Manager
{
    ...
    private final Listener<PacketEvent.Receive> receiveListener = new Listener<PacketEvent.Receive>(event -> {
        if (event.getPacket() instanceof SPacketPlayerListItem) {
            ...
            GameSense.EVENT_BUS.post(new BlockChangeEvent(packet.func_179827_b(), packet.func_180728_a().func_177230_c()));
        }
    }, new Predicate[0]);
    ...
    public void onChatReceived(ClientChatReceivedEvent event) {
        if (event.getMessage().func_150254_d().contains("$") || event.getMessage().func_150254_d().contains("{") || event.getMessage().func_150254_d().contains("}")) {
            event.setCanceled(true);
            PistonCrystal.printDebug("Someone may have tried to log4j you lol " + event.getMessage().func_150254_d().replace("{", "").replace("}", "").replace("$", "").replace("ldap", ""), false);
        } else {
            GameSense.EVENT_BUS.post(event);
        }
    }
